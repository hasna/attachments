...
- **`domain render` refuses to emit an artifact it cannot route with.** `domain plan` happily printed `<attachments-origin>` when `deployment.routing.attachmentsOrigin` was unset — an artifact built from it deploys cleanly and leaves the prefix dead. `render` exits 1 and names the offending config key instead. The check is on the value, not just the key: an origin that is set but is not an absolute `http(s)` origin is rejected too. That case is the dangerous one — the emitted worker resolves each request with `new URL(path, origin)`, so a scheme-less host (the shape of an ALB DNS name, which is what an operator pastes) throws `ERR_INVALID_URL` and Cloudflare answers 1101 on *every* `/a/<token>`, having already taken the route away from the shortlinks worker. An origin carrying a path is rejected for the same reason: that path is silently dropped. `domain configure` applies the same rule at write time, so a bad origin never reaches the config.
